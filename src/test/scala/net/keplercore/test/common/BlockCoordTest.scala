...
class BlockCoordTest extends FlatSpec with Matchers {
	...
	it should "offset properly" in
	{
		val coord = new BlockCoord
		coord offset(1, 2, 3);
		coord shouldEqual new BlockCoord(1, 2, 3)
	}
	...
	it should "offset with a ForgeDirection properly" in
	{
		...
		coord shouldEqual new BlockCoord(0, 0, -1)
	}
	...
	it should "offset with ForgeDirection and a distance properly" in
	{
		...
		coord shouldEqual new BlockCoord(0, 0, -2)
	}
	
	it should "offset with a BlockCoord properly" in
	{
		val coord = new BlockCoord
		coord offset new BlockCoord(1, 2, 3);
		coord shouldEqual new BlockCoord(1, 2, 3)
	}
	...
	{
		val coord = new BlockCoord
		coord getAdjacentCoord ForgeDirection.NORTH shouldEqual new BlockCoord(0, 0, -1)
	}
	...
	it should "get the adjacent coords list properly" in
	{
		...
		coord.getAdjacentCoords shouldEqual new ArrayList[BlockCoord](
			Arrays.asList(
				new BlockCoord(-1, 0, 1), new BlockCoord(-1, 1, 0),
				new BlockCoord(0, -1, 1), new BlockCoord(0, 1, -1),
				new BlockCoord(1, -1, 0), new BlockCoord(1, 0, -1)))
	}
	...
	{
		new BlockCoord(ForgeDirection.NORTH) shouldEqual (new BlockCoord(0, 0, -1))
	}
